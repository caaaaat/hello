...
class Weixin
{
    ...
    public function getToken()
    {
        ...
        if($this->cacheType=='file'){
            ...
            $token      = checkFileDateTime($tokenFile,600);
        }else{
            //数据库模式
            ...
            if($result['access_token'])
            {
                ...
                if($this->cacheType=='file'){
                    ...
                    $updateData = array(
                        ...
                        'lostTime'=>date("Y-m-d H:i:s",time()+600)
                    );
    ...
    public function getJsTicket()
    {
        ...
        if($this->cacheType=='file'){
            ...
            $ticket      = checkFileDateTime($ticketFile,600);
        }else{
            //数据库模式
            $noTime     = date("Y-m-d H:i:s",time());
            $mod        = model('sys.auth','mysql');
    ...
    public function getUserInfoByWeb()
    {
        error_reporting(0);
        $appid     = $this->appId;
        $returnUrl = 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING'];
