...
class ApiWxPayModel extends Model
{
    //微信支付回调处理
    public function wxPayResult($data){
        if($data->return_code == 'SUCCESS'){
            if($data->out_trade_no && $data->result_code == 'SUCCESS'){
                $order = $this->table('order_line a')->jion('left join order_line_discount b on a.id=b.dis_orderId and b.dis_orderStatus=2')->where(array('a.coder'=>$data->out_trade_no))->getOne();
                //writeLog('订单及抵用卷信息：');
                //writeLog($order);
                if($order['status'] == 2){
                    $this->table('order_line')->where(array('coder'=>$data->out_trade_no))->update(array('status'=>6));
                    ...
                    //支付记录
                    $PMoney = $order['totalMoney']-$order['dis_ponMoney'];
                    if($PMoney < 1) $PMoney = 1 ;//实付金额
                    $payArr = array(
                        'orderId'=>$order['id'],
                        'coder'=>$order['coder'],
                        'name'=>'微端全款',
                        'money'=>$PMoney,
                        'supplierGetMoney'=>$order['supplierMoney'],
                    ...
                    $this->table('order_line_pay')->insert($payArr);
                    if($order['dis_type'] ==1 && $order['dis_id'] != ''){
                        $this->table('order_line_discount')->where(array('dis_orderId'=>$order['id'],'dis_id'=>$order['dis_id']))->update(array('dis_orderStatus'=>6,'dis_ponStatus'=>3,'dis_endTime'=>date('Y-m-d H:i:s')));
                        $ponMo = model('sys.coupon','mysql');
                        //$ponMo->useCoupon($typeId,$memberId,$coupon_coder);
                        $ponMo->useCoupon($order['dis_type'],$order['dis_memberId'],$order['dis_ponId']);//修改抵用卷状态
                    }

                    //供应商 资金流
                    $supplierMoney = $order['supplierMoney'];
                    $about = '微信端全款：'.'订单：'.$order['coder'].', 产品：'.$order['title'];
                    $mo->addFundInfo($order['supplierId'], $order['supplierName'], '', '', '', '', '微信端全款', $about, $supplierMoney, 1);//写入资金日志
                    ...
                    if($order['storeId'] && $storeMoney > 0){
                        $mo->addFundInfo($order['storeId'], $order['storeName'], $order['userId'], $order['userName'], '', '', '微信端全款', $about, $storeMoney, 1);
                    }
                    //系统消息
                    $sendMsg  = '支付通知：您有一条订单已支付全部金额，订单号：'.$order['coder'].'。   ';
                    ...
                    foreach($reviewWx as $wx){
                        if ($wx['wxOpenId']) {
                            ...
                            $wxData = array(
                                'first' => '尊敬的'.$wx['wxName'] . ', 您的产品 "' .$order['title']. '" 游客已支付全款',//标题抬头
                    ...
                    if($tels){
                        //用户短信提醒
                        //发短信
                        $notice  = '尊敬的'.$order['linkMan'] .',感谢您选择四川邮电旅游的线路产品,您的订单"'.$order['title'] . '"已支付成功。';
                    ...
                    if ($memberId) {
                        ...
                        $notice  = '尊敬的'.$member['nickName'] .',感谢您选择四川邮电旅游的线路产品,您的订单"'.$order['title'] . '"已支付成功。';
                        $notice .= '出游日期：'.$order['startDay'].',我们将通过短信为您发送出团通知,请注意查收。';
                        $notice .= '预祝您旅途愉快！';
                        $url     = '';
                        $wxData  = array(
     ...
     * 取得支付信息
     * @param $billId
     * @param $payType  1到店付，2支付宝，3微支付,4银行卡
     */
    public function payItem($billId,$payType,$ponId)
    {
        //支付体系
        //+-------------------------------------
        //到店支付
       ...
       /* if($payType=='2')
        ...
        if($payType=='3')
        {
            $return = $this->checkPayWeiXin($billId,$ponId);
            return $return;
        }
     ...
     * @param $disId
     */
    public function checkPayWeiXin($billId,$ponId)
    {
        ini_set('date.timezone','Asia/Shanghai');
        $find  = array('id'=>$billId);
        $rows  = $this->table('order_line')->where($find)->getOne();
        ...
        if($rows['status']=='2')
        {
            ...
            //writeLog($money);
            if( $ponId){
                $money      = $this->discountType($rows,$ponId);//返回优惠后的应付金额
            }
            //writeLog('使用抵用卷后的金额:');
            //writeLog($money);
            ...
            $input->SetNotify_url("http://pro.scydgl.com/pay/wx");
            ...
            if($orderError=='OUT_TRADE_NO_USED'){
                //重新生成订单号
                $billCoder  = date("YmdHis").rand(1000,9999);
                //修改订单编号
                $updata     = array('coder'=>$billCoder);
                $this->table('order_line')->where(array('id'=>$billId))->update($updata);
        ...
        if(empty($order) || $order['status']=='10'){
            ...
            if($order['money'] - $order['supplierTuiMoney'] <= 0){//如果支付金额小于团损 则直接 修改成退款完成
                ...
                if($rst){
                    ...
                    //微信用户 退款后 从质量跟踪中删除
                    $this->table('server_user')->where(array('orderCoder' =>$order['coder'],'proType'=>1))->del();
