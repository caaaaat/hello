...
//------------------------------------------------------用户体系
    ...
    */
    ...
    function writeCursor($filename,$content){
        //判断文件夹是否存在，不存在的话创建
        if(!file_exists(APPROOT.'/data/huanxin/')){
            mkdir(APPROOT.'/data/huanxin/');
        }
        $myfile=@fopen(APPROOT.'/data/huanxin/'.$filename,"w+") or die("Unable to open file!");
    ...
    function readCursor($filename){
        ...
        $file=APPROOT.'/data/huanxin/'.$filename;
    ...
    */
     ...
     * @param $target_type 消息发送类型 users 给用户发消息。chatgroups: 给群发消息，chatrooms: 给聊天室发消息
     * @param $target
     * @param $content
     * @param $ext
     * @return mixed
